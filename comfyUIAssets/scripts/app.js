...
export class ComfyApp {
	...
	async #loadExtensions() {
	    ...
	    const extensionPromises = extensions.map(async ext => {
	        try {
				// @@@ ESPM
	            await import(api.extensionsURL(ext));
	        } catch (error) {
	...
	async setup() {
		...
		// @@@ ESPM
		document.getElementById("comfy-about-button").click();
	}
	...
	#formatPromptError(error) {
		...
		// @@@ ESPM
		else if (error.response && typeof error.response === "string") { return error.response; }
		else if (error.response && error.response.message && typeof error.response.message === "string") { return error.response.message; }
	...
	async queuePrompt(number, batchCount = 1) {
		...
		// @@@ ESPM
		this.#queueItems.push({ number, batchCount: 1 });
